import type { ASTNode, FragmentDefinitionNode, DirectiveNode } from "graphql";
...
export type ScalarsMap =
  | string
  | { [name: string]: string | { input: string; output: string } };
/**
...
export type EnumValuesMap<AdditionalProps = {}> =
  ...
  | {
      [enumName: string]:
        | string
        | ({ [key: string]: string | number } & AdditionalProps);
    };
...
export type GetFragmentSuffixFn = (
...
export type ConvertFn<T = {}> = (
  node: ASTNode | string,
  options?: ConvertOptions & T
) => string;
export type NamingConvention = "keep";
