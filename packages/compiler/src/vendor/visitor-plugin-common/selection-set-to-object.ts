...
type CollectedFragmentNode = (
  | SelectionNode
  | FragmentSpreadUsage
  | DirectiveNode
) &
  FragmentDirectives;
type GroupedStringifiedTypes = Record<
  string,
  Array<string | { union: string[] }>
>;
...
> {
  ...
  _collectInlineFragments(
    ...
    if (isListType(parentType) || isNonNullType(parentType)) {
      return this._collectInlineFragments(
        parentType.ofType as GraphQLNamedType,
        nodes,
        types
      );
    }
    if (isObjectType(parentType)) {
      for (const node of nodes) {
        ...
        const fieldsWithFragmentDirectives: CollectedFragmentNode[] =
          fields.map((field) => ({
            ...field,
  ...
  protected _buildGroupedSelections(parentName: string): {
    ...
    if (
      !this._config.mergeFragmentTypes ||
      this._config.inlineFragmentTypes === "mask"
    ) {
      const grouped = possibleTypes.reduce<GroupedStringifiedTypes>(
        (prev, type) => {
          ...
          const { incrementalNodes, selectionNodes, fragmentSpreads } =
            allNodes.reduce<{
          ...
          const { fields, dependentTypes: subDependentTypes } =
          ...
          for (const incrementalNode of incrementalNodes) {
            if (
              ...
              const {
                ...
                dependentTypes: incrementalDependentTypes,
            ...
            const {
              ...
              dependentTypes: subsequentDependentTypes,
            } = this.buildSelectionSet(schemaType, [incrementalNode], {
              unsetTypes: true,
              parentFieldName: parentName,
            });
            ...
            const subsequentSet =
              this.selectionSetStringFromFields(subsequentFields);
            dependentTypes.push(
    ...
    const compacted = Object.keys(grouped).reduce<Record<string, string[]>>(
      (acc, key) => {
  ...
  protected selectionSetStringFromFields(
    ...
    const allObjects = fields
      .filter(
        (f: string | NameAndType): f is NameAndType => typeof f !== "string"
      )
      .map((t) => `${t.name}: ${t.type}`);
  ...
  protected buildSelectionSet(
    ...
    for (const selectionNode of selectionNodes) {
      ...
      if (
        ...
        parentSchemaType
          .getInterfaces()
          .find((iinterface) => iinterface.name === selectionNode.onType) !=
          null ||
        (isUnionType(fragmentType) &&
          fragmentType
            .getTypes()
            .find((objectType) => objectType.name === parentSchemaType.name))
      ) {
        // also process fields from fragment that apply for this parentType
        const flatten = this.flattenSelectionSet(
    ...
    for (const {
      ...
      const selectionSetObjects = selectionSet.transformSelectionSet(
        options.parentFieldName
          ? `${options.parentFieldName}_${fieldName}`
          : fieldName
      );

      linkFieldsInterfaces.push(...selectionSetObjects.dependentTypes);
      const isConditional =
        hasConditionalDirectives(field) || inlineFragmentConditional;
      ...
      .map((typeName) => {
        ...
        return relevant.map((objDefinition) => {
          ...
          return {
            name,
            content:
              typeof objDefinition === "string"
                ? objDefinition
                : objDefinition.union.join(" | "),
            isUnionType: !!(
              typeof objDefinition !== "string" &&
              objDefinition.union.length > 1
            ),
          };
        });
      })
      ...
      ...dependentTypes.map((pair) =>
        pair
          .map(({ name, content, isUnionType }) =>
            ...
            this._config.extractAllFieldsToTypes
              ? name
              : isUnionType
              ? `(${content})`
              : content
  ...
  public transformFragmentSelectionSetToTypes(
    ...
    const mergedTypeString = this.buildFragmentTypeName(
      fragmentName,
      fragmentSuffix
    );
    const { grouped, dependentTypes } =
      this._buildGroupedSelections(mergedTypeString);

    const subTypes: DependentType[] = Object.keys(grouped).flatMap(
      (typeName) => {
        ...
        if (possibleFields.length === 0) {
          if (!this._config.addTypename) {
            return [
              { name: declarationName, content: this.getEmptyObjectType() },
            ];
          }
        ...
        const content =
          flatFields.length > 1
            ? flatFields
                .map(({ value, isUnionType }) =>
                  isUnionType ? `(${value})` : value
                )
                .join(" & ")
  ...
  protected buildParentFieldName(typeName: string, parentName: string): string {
    ...
    return operationTypes.includes(typeName)
      ? parentName
      : `${parentName}_${typeName}`;
  }
}
